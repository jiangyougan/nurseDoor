...
<div class="pd-20">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<a class="btn btn-primary radius" onclick="picture_add('添加图片','goBackdRollpicadd.action')" href="javascript:;">
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
				...
				<tr style="text-align: center;">
                	...
                	<img style="width: 120px;height: 120px" alt="" src="<%=webPath%>${s.rollPicUrl }">
                	</td>
					<td style="text-align: center;">${s.rollPicNote }</td>
					<td style="text-align: center;">${s.rollPicSpeak }</td>
					<td style="text-align: center;">${s.rollPicCreater }</td>
                	<td style="text-align: center;">
                    	<fmt:formatDate value="${s.rollPicCreatetime }" type="both"/>
               		 </td>
                <td style="text-align: center;">
                    ...
                    <a title="删除" href="javascript:;" onclick="project_del(this,'${s.rollPicId}')"
                       class="ml-5" style="text-decoration:none">
                        <i class="Hui-iconfont">&#xe6e2;</i>
                    </a>
                    <c:if test="${(z.index) != 0 }">
                     <a title="向上移动" href="javascript:;" onclick="content_upwardone('${s.rollPicId}','${s.rollPicSpeak}','${z.index}')"
                       class="ml-5" style="text-decoration:none">
   ...
   /*  function project_add(title, url, w, h) {
    ...
    /*管理员-删除*/
     function project_del(obj, id) {
        layer.confirm('确认要删除吗？', function (index) {
            //此处请求后台程序，下方是成功后的前台处理……
            location.href = "${pageContext.request.contextPath}/khdroolPicdel.action?rollPicId=" + id;
            $(obj).parents("tr").remove();
            layer.msg('已删除!', {icon: 1, time: 1000});
        });
    } 
...
function picture_add(title,url){
	...
	layer.full(index); */
	location.href = "${pageContext.request.contextPath}/goBackdRollpicadd.action";
