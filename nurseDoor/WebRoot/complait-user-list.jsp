...
<div class="pd-20">
	...
	<ul class="nav nav-tabs" role="tablist">
  		...
  		<li role="presentation" class="active"><a href="${pageContext.request.contextPath}/complaintUserlook.action">用户投诉反馈平台</a></li>
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		<form action="${pageContext.request.contextPath}/complaintUserlookKeyWord.action" method="post">
			<div class="text-c">日期范围：
    ...
    <nav>
      <ul class="pagination">
        ...
        <c:if test="${pageNow > 1}">
         <li style="float:left">
          <a href="${pageContext.request.contextPath}/complaintUserlook.action?pageNow=${pageNow-1}" aria-label="Previous">
       ...
       <c:forEach begin="${start}" end="${end}" var="p">
		   ...
		   <c:if test="${p != pageNow &&  p != 0}">
	         <li><a href="${pageContext.request.contextPath}/complaintUserlook.action?pageNow=${p}">${p}</a></li>
	       </c:if>
       </c:forEach> 
       <c:if test="${totalPage > pageNow}">
        <li>
          <a href="${pageContext.request.contextPath}/complaintUserlook.action?pageNow=${pageNow+1}" aria-label="Next">
            <span aria-hidden="true">»</span>
          </a>
        </li>
       </c:if> 
