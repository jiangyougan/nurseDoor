...
<!-- <form action="" method="post">
	<div class="text-c"> 日期范围：
		...
		<button type="submit" class="btn btn-success radius" id="" name=""><i class="Hui-iconfont">&#xe665;</i>搜投诉</button>
	</div>
</form>	 -->
	<ul class="nav nav-tabs" role="tablist">
  		...
  		<li role="presentation" ><a href="${pageContext.request.contextPath}/complaintUserlookToNurse.action">用户投诉反馈护士</a></li>
  		<li role="presentation" class="active"><a href="${pageContext.request.contextPath}/selectComplaintByUserTo.action">订单投诉反馈</a></li>
	</ul>
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		<form action="${pageContext.request.contextPath}/selectComplaintByUserToKeyWord.action" method="post">
			<div class="text-c">日期范围：
		...
		<table class="table table-border table-bordered table-hover table-bg table-sort">
			<thead>
				<tr class="text-c">
					...
					<th nowrap="nowrap" width="60">护士昵称(投诉人)</th>
					<th nowrap="nowrap" width="60">用户昵称(被投诉人)</th>
					...
					<th nowrap="nowrap" width="20">投诉类型</th>
			...
			<tbody>
				<c:forEach items="${list }" var="s" varStatus="z">
					<tr class="text-c">
						...
						<%-- <td><a href="javascript:;"><i class="avatar size-L radius"><img alt="" src="<%=webPath %>${s.nurseHeader }"></i></a></td> --%>
						
						<td class="text-l" style="text-align: center !important;">
							<c:if test="${s.complaintSparethree != '' && s.complaintSparethree != null}">
								<div class="c-999 f-12">
									<u style="cursor:pointer" class="text-primary" onclick="member_show('个人信息','nurse-info-show.jsp?nurseId=${s.nurseId}','360','400')">${s.userNick }</u> 
								</div>
							</c:if>		
						</td>
						...
						<th width="30"><a href="OrderInfoByOrderId.action?orderSendId=${s.complaintContentNote}">${s.orderSendNumber }</a></th>
						...
						<th width="60">
							...
							<c:set var="result" value="${fn:split(imgsp, ',')}" />
									<c:forEach items="${result}" var="primg">
										<img alt="没有图片" src="<%=host %>${primg}" width="60px" height="60px">
									</c:forEach>
						</th>
						...
						<td class="td-status">
						<c:if test="${s.complaintSparethree != '' && s.complaintSparethree != null}">
						<span class="label label-warning radius">客服新建</span>
						</c:if>
						<c:if test="${s.complaintSparetwo == '0' }">
						<span class="label label-success radius">取消反馈</span>
						</c:if>
						<c:if test="${s.complaintSparetwo == '1'}">
						<span class="label label-success radius">完成反馈</span>
						</c:if>
						<c:if test="${s.complaintSpareone == '0' }">
						<span class="label label-success radius">待付款沟通</span>
						</c:if>
						</td>
						...
						<%if(adminUsergrade <= 1){%>
						<td class="td-manage">
						<a title="标记已审核" href="javascript:;" onclick="handleComplait(this,'${s.complaintId}',${s.complaintContentOver})" style="text-decoration:none"><img src="img/checkmark18.png" style="width: 12px;height: 12px"/></a>
						</td>
						<%} %>
					</tr>
				</c:forEach>	
    ...
    <nav>
      <ul class="pagination">
        ...
        <c:if test="${pageNow > 1}">
         <li style="float:left">
          <a href="${pageContext.request.contextPath}/selectComplaintByUserTo.action?pageNow=${pageNow-1}" aria-label="Previous">
       ...
       <c:forEach begin="${start}" end="${end}" var="p">
		   ...
		   <c:if test="${p != pageNow &&  p != 0}">
	         <li><a href="${pageContext.request.contextPath}/selectComplaintByUserTo.action?pageNow=${p}">${p}</a></li>
	       </c:if>
       </c:forEach> 
       <c:if test="${totalPage > pageNow}">
        <li>
          <a href="${pageContext.request.contextPath}/selectComplaintByUserTo.action?pageNow=${pageNow+1}" aria-label="Next">
            <span aria-hidden="true">»</span>
          </a>
        </li>
       </c:if> 
...
//订单详情
if(id == null || '' == id){
layer.msg('没有订单信息!',{icon: 6,time:1000});
}else{
location.href = "${pageContext.request.contextPath}/OrderInfoByOrderId.action?orderSendId=" + id;
}
}
/*密码-修改*/
...
//处理投诉订单
/* function handleComplait(obj,id,complaintContentOver){
alert(complaintContentOver);
if(complaintContentOver == '0'){
...
function handleComplait(obj,id,complaintContentOver){
if(complaintContentOver == '0'){
layer.msg('请处理之后再审批!', {icon: 6, time: 1500});
return;
}
var complaintId = id;
	layer.confirm('确认已处理吗？',function(index){
