...
<%-- <ul class="nav nav-pills" role="tablist">
  <li role="presentation" class="active"><a href="${pageContext.request.contextPath}/selectcostTraffic.action">交通费</a></li>
...
<form method="post" class="form form-horizontal" id="form-article-add"onsubmit="return validate(document.getElementById('contentName'));">
			<div class="row cl">
				<label class="form-label col-2"><span class="c-red">*</span>待处理订单时间控制：</label>
				<div class="formControls col-4">
					<input type="text" style="width: 50%" class="input-text"  value="${Timing }" placeholder="" id="TimingControl" name="TimingControl"/>分钟
				</div>
			</div>
...
//利率输入框的限制条件
$("input[name*='TimingControl']").keyup(function () { 
...
<script type="text/javascript">
     ...
     var TimingControl = $("#TimingControl").val();
     if(TimingControl == null){
     alert("为空默认为0分钟");
     return;
     }
     		$.ajax({
						url: "${pageContext.request.contextPath}/insertTimingControl.action",              //请求地址
				        type: "POST",                       //请求方式
				        data: { TimingControl: TimingControl},        //请求参数
				        dataType: "json",
