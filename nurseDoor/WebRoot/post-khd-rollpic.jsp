...
<div class="pd-20">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<a class="btn btn-primary radius" onclick="picture_add('添加学术圈图片','PostgoBackdRollpicadd.action')" href="javascript:;">
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
				...
				<tr style="text-align: center;">
                	...
                	<img style="width: 120px;height: 120px" alt="" src="<%=webPath%>${s.postRollPicUrl }">
                	</td>
					<td style="text-align: center;">${s.postRollPicNote }</td>
					<td style="text-align: center;">${s.postRollPicSpeak }</td>
					<td style="text-align: center;">${s.postRollPicCreater }</td>
                	<td style="text-align: center;">
                    	<fmt:formatDate value="${s.postRollPicCreatetime }" type="both"/>
               		 </td>
                <td style="text-align: center;">
                    ...
                    <a title="删除" href="javascript:;" onclick="project_del(this,'${s.postRollPicId}')"
                       class="ml-5" style="text-decoration:none">
                        <i class="Hui-iconfont">&#xe6e2;</i>
                    </a>
                    <c:if test="${(z.index) != 0 }">
                     <a title="向上移动" href="javascript:;" onclick="content_upwardone('${s.postRollPicId}','${s.postRollPicRank}','${z.index}')"
                       class="ml-5" style="text-decoration:none">
...
//上移下移
  function content_upwardone(rollPicId,order,index) {
          location.href = "${pageContext.request.contextPath}/postRollPicupwardone.action?rollPicId=" + rollPicId+"&order="+order+"&index="+index;
    }
   ...
   /*  function project_add(title, url, w, h) {
    ...
    /*管理员-删除*/
     function project_del(obj, id) {
        layer.confirm('确认要删除吗？', function (index) {
            //此处请求后台程序，下方是成功后的前台处理……
            location.href = "${pageContext.request.contextPath}/roolPicdel.action?rollPicId=" + id;
            $(obj).parents("tr").remove();
            layer.msg('已删除!', {icon: 1, time: 1000});
        });
    } 
...
function picture_add(title,url){
	...
	layer.full(index); */
	location.href = "${pageContext.request.contextPath}/PostgoBackdRollpicadd.action";
