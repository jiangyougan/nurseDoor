...
<body>
<div class="pd-20">
  <form action="${pageContext.request.contextPath}/UpdatecancelOrdenComplaint.action" method="post" class="form form-horizontal" name="form1" id="form1" enctype="multipart/form-data" >
  <input type="hidden" value="<%=orderSendId %>" id="orderSendId" name="orderSendId"/>
  <input type="hidden" value="<%=complaintSparetwo %>" id="complaintSparetwo" name="complaintSparetwo"/>
  <input type="hidden" value="${complaint.complaintUserid }" id="userId" name="userId"/>
  <div class="row cl">
    ...
    <div class="row cl">
     ...
     <c:if test="${complaint.complaintSparetwo == '0'}">
    <div class="row cl">
      <label class="form-label col-3">处理方式：</label>
      <div class="formControls col-5"> <span class="select-box">
        <select class="select" size="1" id="brandclass" name="demo1" datatype="*" nullmsg="请选择处理方式！">
          <option value="" selected>请选择类别(不选则默认原护士处理)</option>
          <option value="0">退款</option>
          <option value="1">换护士</option>
          <option value="2">原人处理</option>
     ...
     <input type="hidden" id="selectVal" name="selectVal" value="${complaint.complaintHandleMode}" >
     <input type="hidden" id="state" name="state" value="${state}" >
  <!--   <div class="row cl">
      <label class="form-label col-3">备注：</label>
      <div class="formControls col-5">
        onKeyUp="textarealength(this,100)"
         <textarea id="note" name="note" cols="" rows="" class="textarea"  placeholder="谈话内容" datatype="*10-100" dragonfly="true" ></textarea>
...
function onreturn(){
var state = $("#state").val();
var returnValue = $("#complaintSparetwo").val();
if(state == '5'){
location.href = "${pageContext.request.contextPath}/OrderList.action?state=5";
}else{
if(returnValue == 0){
