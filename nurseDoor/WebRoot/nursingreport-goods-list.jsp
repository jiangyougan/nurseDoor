...
<form action="${pageContext.request.contextPath}/OrderKeyWordList.action" method="post">
	...
	<div class="mt-20">
	<!-- <input type="submit" name="actionButton" value="应用" > -->
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
				<c:forEach items="${mNursingTestings }" var="s"  varStatus="z">
				<tr style="text-align: center;">
			 		<td style="text-align: center;"><input type="checkbox" value="${s.manageNursingTestingId}" name="ifAll" id="ifAll" onClick="checkAll(this)"/></td>
                	<td style="text-align: center;">${s.testallName }</td>
					<td style="text-align: center;">${s.testallUnit }</td>
				
                	<td style="text-align: center;">
                    	<fmt:formatDate value="${s.manageNursingTestingCreateter }" type="both"/>
               		 </td>
					<td style="text-align: center;">
                	 	
                    <a title="删除" href="javascript:;" onclick="project_del(this,'${s.manageNursingTestingId}','${goodsId }')"
...
/*
    ...
    /*管理员-删除*/
    function project_del(obj, id,goodsId) {
    	
        layer.confirm('确认要删除吗？', function (index) {
            //此处请求后台程序，下方是成功后的前台处理……
            location.href = "${pageContext.request.contextPath}/delecttestingbyid.action?testingId=" + id+"&goodsId="+goodsId;
            $(obj).parents("tr").remove();
            layer.msg('已删除!', {icon: 1, time: 1000});
        });
    }
