...
<ul class="nav nav-pills" role="tablist">
  ...
  <li role="presentation"><a href="${pageContext.request.contextPath}/capitaluserinfo.action">用户资金明细</a></li>
  <li role="presentation" class="active"><a href="${pageContext.request.contextPath}/capitalnurseinfo.action">护士资金明细</a></li>
</ul>
			...
			<tbody>
				<c:forEach items="${list}" var="s">
					<tr class="text-c">
						...
						<c:if test="${s.capitalNursePoolPeopleidIdentity == 0}">
							<td class="td-status"><span class="label label-error radius">支出</span></td>
						</c:if>
						<c:if test="${s.capitalNursePoolPeopleidIdentity == 1}">
							<td class="td-status"><span class="label label-success radius">收益</span></td>
						</c:if>
						
						
						<td class="text-c">${s.capitalNursePoolMoney}￥</td>
						
						<c:if test="${s.capitalNursePoolType == 0}">
							<td class="td-status"><span class="label label-success radius">注册</span></td>
						</c:if>
						<c:if test="${s.capitalNursePoolType == 1}">
							<td class="td-status"><span class="label label-error radius">认证</span></td>
						</c:if>
						
						<c:if test="${s.capitalNursePoolType == 2}">
							<td class="td-status"><span class="label label-success radius">推荐</span></td>
						</c:if>
						<c:if test="${s.capitalNursePoolType == 3}">
							<td class="td-status"><span class="label label-success radius">开单</span></td>
						</c:if>
						<c:if test="${s.capitalNursePoolType == 4}">
							<td class="td-status"><span class="label label-success radius">返利</span></td>
						</c:if>
						
						<c:if test="${s.capitalNursePoolType == 5}">
							<td class="td-status"><span class="label label-success radius">首单</span></td>
						</c:if>
						<c:if test="${s.capitalNursePoolType == 6}">
							<td class="td-status"><span class="label label-success radius">提现</span></td>
						</c:if>
						
						<c:if test="${s.capitalNursePoolType == 7}">
							<td class="td-status"><span class="label label-success radius">完成订单</span></td>
						</c:if>
						
						<td class="text-c">${s.capitalNursePoolSpeak }</td>
						
						<td >
							<fmt:formatDate value="${s.capitalNursePoolCreatetime}" type="both"/>
						</td>
						<c:if test="${s.capitalNursePoolType == 7}">
							<td class="td-manage">
								<a style="text-decoration:none" class="ml-5" onClick="picture_edit('订单详情','OrderInfoByOrderId.action?orderSendId=${s.capitalNursePoolOrdersendid}','100001')" href="javascript:;" title="编辑">
								<img alt="" src="images/search-icon.png" style="width: 15px;height: 15px"></a>
								
								<a style="text-decoration:none" class="ml-5" onclick="member_show('个人信息','nurse-info-show.jsp?nurseId=${s.capitalNursePoolPeopleid}','10001','360','400')" href="javascript:;" title="编辑">
								<i class="Hui-iconfont">&#xe6df;</i></a>
							</td>
						</c:if>
