...
<div class="pd-20">
	<form action="" method="post" class="form form-horizontal" id="form-article-add"onsubmit="return validate(document.getElementById('contentName'));">
		<div class="row cl">
			<label class="form-label col-2"><span class="c-red">*</span>一级名称：</label>
			<div class="formControls col-10">
				<input type="text" style="width: 50%" class="input-text" value="" placeholder="" id="postOneName" name="postOneName"/>
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-2"><span class="c-red">*</span>备注：</label>
			<div class="formControls col-10">
				<input type="text"  style="width: 50%" class="input-text" value="" placeholder="" id="postOneNote" name="postOneNote" />
			</div>
		</div>
...
var orderSendId = $("#orderSendId").val();
	//一级名称
	var postOneName = $("#postOneName").val();
	var postOneNote = $("#postOneNote").val();
	//当前时间
		if(postOneName==null || postOneName==""){
			layer.msg('名称不能为空!',{icon: 5,time:2000});
		}else{
				/* $("#form-article-add").submit(); */
				$.ajax({
					url: "${pageContext.request.contextPath}/postOneLevelAdd.action",              //请求地址
			        type: "POST",                       //请求方式
			        data: { postOneName: postOneName,postOneNote: postOneNote},        //请求参数
			        dataType: "json",
